...
SIZE_REGS_BLOCK = 64  # 9 regs de 2B + FLAGS 1B + PID 2B + FPID 2B + RING 1B
...
def load(G_MEM: bytearray, G_MMU: list, PID: int, sectors, BIN: bytearray, FPID, RING, modid):
    ...
    # regs_offset = inicio del PCB

    # F_exec = 0x0101
    G_MEM[regs_offset + 0] = 0x01  # low byte
    G_MEM[regs_offset + 1] = 0x01  # high byte

    # MOD_ID
    G_MEM[regs_offset + 2] = modid

    # RING
    G_MEM[regs_offset + 3] = RING

    # PID
    G_MEM[regs_offset + 4] = PID & 0xFF
    G_MEM[regs_offset + 5] = (PID >> 8) & 0xFF

    # FPID
    G_MEM[regs_offset + 6] = FPID & 0xFF
    G_MEM[regs_offset + 7] = (FPID >> 8) & 0xFF
