...
def calls(regs):
    ...
    if call == 0x00:
        ...
        if callcode == 0x00:
            ...
            osx = regs[2]
            osy = regs[3]
            w = (G_MEM[os+mainoffset+1]<<8) | G_MEM[os+mainoffset]
            h = (G_MEM[os+mainoffset+3]<<8) | G_MEM[os+mainoffset+1]
            for i in range(w):
                for j in range(h):
                    screen.set_at(
                        (
                            ...
                            G_MEM[imgoffset + 3*(i + j*w)],
                            G_MEM[imgoffset + 3*(i + j*w)+1],
                            G_MEM[imgoffset + 3*(i + j*w)+2],
                        )
                    )
        elif callcode == 0x04:
            w = (G_MEM[regs[1]+mainoffset+1]<<8) | G_MEM[regs[1]+mainoffset]
